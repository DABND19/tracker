...
class EmployeeController:
    ...
    async def delete(self, user: types.User):
        await self._session.execute(delete(Employee).where(Employee.id == user.id))
    ...
    async def get_admins_ids(self) -> List[int]:
        result = await self._session.execute(select(Employee.id).where(Employee.is_superuser == True))
        return list(map(lambda record: record[0], result.all()))
